// Port of Vladimir Agafonkin's SunCalc library for Project Zenith
...
export function getMoonPosition(date: Date, lat: number, lng: number): MoonPosition {
  ...
  // Convert azimuth from radians (-PI to PI) to degrees (0 to 360)
  // SunCalc returns azimuth where 0 is South, -PI is East, PI is West
  // Let's standardise to 0 is South, clockwise to 360:
  let azDeg = (azimuth(H, phi, c.dec) * 180) / PI + 180;
  
  return {
    azimuth: azDeg % 360,
    altitude: (h * 180) / PI,
    distance: c.dist,
    parallacticAngle: (pa * 180) / PI,
  };
}
...
export function getMoonTimes(date: Date, lat: number, lng: number, inUTC = false): MoonTimes {
  ...
  const hc = 0.133 * rad;
  let h0 = getMoonPosition(t, lat, lng).altitude * rad - hc;
  let h1: number;
  let h2: number;
  ...
  for (let i = 1; i <= 24; i += 2) {
    h1 = getMoonPosition(hoursLater(t, i), lat, lng).altitude * rad - hc;
    h2 = getMoonPosition(hoursLater(t, i + 1), lat, lng).altitude * rad - hc;
