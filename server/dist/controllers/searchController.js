...
class SearchController {
    static async getSearchHistory(req, res) {
        try {
            ...
            const history = await SearchHistory_1.SearchHistory.find({ userId: req.userId })
                .sort({ createdAt: -1 })
                .limit(10);
    ...
    static async addSearch(req, res) {
        try {
            ...
            const latest = await SearchHistory_1.SearchHistory.findOne({ userId: req.userId }).sort({ createdAt: -1 });
            ...
            const search = new SearchHistory_1.SearchHistory({
                userId: req.userId,
                query: trimmedQuery
            });
            await search.save();
    ...
    static async clearSearchHistory(req, res) {
        try {
            if (!req.userId)
                return res.status(401).json({ message: 'Unauthorized' });
            await SearchHistory_1.SearchHistory.deleteMany({ userId: req.userId });
