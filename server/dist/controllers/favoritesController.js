...
class FavoritesController {
    static async getFavorites(req, res) {
        try {
            ...
            const list = await Favorite_1.Favorite.find({ userId: req.userId }).sort({ createdAt: -1 });
    ...
    static async addFavorite(req, res) {
        try {
            ...
            const existing = await Favorite_1.Favorite.findOne({ userId: req.userId, objectId, objectType });
            if (existing) {
                return res.json({
            ...
            const fav = new Favorite_1.Favorite({
                userId: req.userId,
                objectId,
                objectType
            });
