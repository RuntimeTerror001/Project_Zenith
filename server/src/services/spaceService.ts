...
async function fetchWithTimeout(url: string, options: any = {}, timeoutMs = 3000): Promise<any> {
...
function getPlanetHorizontalPositionFallback(
  planet: string,
  date: Date,
  lat: number,
  lng: number
...
function calculateCelestialRiseSet(
  lat: number,
  lng: number,
  ra: number,
  dec: number,
  ...
  const formatLocalTime = (utHour: number) => {
...
export class SpaceService {
  ...
  static async getIssPosition(): Promise<any> {
    ...
    try {
      const openNotifyPosPromise = fetchWithTimeout('http://api.opennotify.org/iss-now.json').then(r => r.json() as Promise<any>).catch(() => null);
      const openNotifyAstrosPromise = fetchWithTimeout('http://api.opennotify.org/astros.json').then(r => r.json() as Promise<any>).catch(() => null);
      const whereTheIssPromise = fetchWithTimeout('https://api.wheretheiss.at/v1/satellites/25544').then(r => r.json() as Promise<any>).catch(() => null);
  ...
  static async getLiveEvents(): Promise<any> {
    ...
    try {
      ...
      const neoPromise = fetchWithTimeout(`https://api.nasa.gov/neo/rest/v1/feed?start_date=${today}&end_date=${today}&api_key=${nasaKey}`).then(r => r.ok ? r.json() as Promise<any> : null).catch(() => null);
      const launchPromise = fetchWithTimeout('https://ll.thespacedevs.com/2.2.0/launch/upcoming/?limit=6').then(r => r.ok ? r.json() as Promise<any> : null).catch(() => null);
      const kpPromise = fetchWithTimeout('https://services.swpc.noaa.gov/products/noaa-planetary-k-index.json').then(r => r.ok ? r.json() as Promise<any> : null).catch(() => null);
   ...
   */
  static async getConstellationsBoundaries(): Promise<any> {
    ...
    try {
      if (fs.existsSync(dataFilePath)) {
        ...
        if (!response.ok) throw new Error('Constellation boundaries fetch error');
        data = await response.json();
        fs.writeFileSync(dataFilePath, JSON.stringify(data));
      }
   ...
   */
  static getConstellationsMythology(): any[] {
   ...
   */
  static async getTimelineEvents(): Promise<any[]> {
    try {
      const list = await TimelineEvent.find().sort({ year: 1 });
      if (list.length > 0) {
        return list.map((ev) => ({
          year: ev.year,
          title: ev.title,
      ...
      if (fs.existsSync(dataFilePath)) {
        ...
        const seededList = await TimelineEvent.find().sort({ year: 1 });
   ...
   */
  static async getPlanetsData(lat: number, lng: number, dateStr: string): Promise<any[]> {
    ...
    await Promise.all(planetList.map(async (planet) => {
      try {
        ...
        if (!res.ok) throw new Error('NASA Horizons request failed');
        const data = await res.json() as any;
   ...
   */
  static calculateNextIssPass(lat: number, lng: number, tle1: string, tle2: string): Date | null {
    try {
   ...
   */
  static async getAstronomyStats(lat: number, lng: number, dateStr: string): Promise<any> {
    const date = new Date(dateStr);
    const dateFormatted = date.toISOString().slice(0, 10);
